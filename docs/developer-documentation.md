...
The Cabal library exposes the modules under `src/`; the four files under `app/`
are thin executable entry points. `Controller.collectActions` connects worm IDs
to agents. `Gui` and `PlayGui` use that interface for interactive Watch and Play
sessions. `Training` and `Evaluation` drive the same game engine without Gloss;
`app/TrainModel.hs` and `app/EvaluateModels.hs` expose configurable command-line
entry points.
...
`cerviq-evaluate` can write text reports below the locally chosen output path.
The repository's `.gitignore` excludes `outputs/`, so reports kept there are
local benchmark artifacts rather than distributed repository files. They are
not automated test reports.

## 17. Repository and source structure

```text
cerviQ.cabal                 exposed library, four executables, and test suite
...
data/models/                 persisted tables and ignored local training stats
test/Spec.hs                 Tasty/HUnit deterministic unit-test suite
outputs/evaluation/          ignored local pairwise report destination
...
| Automated unit tests | `test/Spec.hs`: `main` and the nine `TestTree` groups |
...
The `cerviq-tests` Cabal target runs `test/Spec.hs` with Tasty and HUnit. Its 59
test cases are organized into nine groups:

| Group | Verified behavior |
| --- | --- |
| Movement | Relative turns, reverse rejection, head prediction, and body growth |
| Collision | Walls, poison, bodies, two-/three-way head collisions, and unaffected worms |
| Game | Missing actions, four-worm stepping, food growth, tail vacancy, kills, and statistics |
| Maps | ASCII parsing, dimensions, sparse empty cells, food, and poison |
| Scenario | Built-in validation, default states, capacities, maximum rosters, IDs, and invalid counts |
| Adaptive Food | Scaling, cap, clearing, head distance, dead-worm exclusion, random placement, and fallback |
| Play | Complete multi-opponent running/win/loss/draw result matrix |
| Q-learning Core | Unseen/known entries, tied best actions, and terminal/non-terminal Q updates |
| Evaluation | Sole-survivor detection and persistent last-standing recording |

The suite exercises deterministic rules and invariants; the randomized food
cases assert properties rather than fixed coordinates. It does not currently
exercise Gloss rendering/input, Watch history, viewport geometry, controller IO,
model file round trips, complete V1-V4 encoders/policies, or long stochastic
training/evaluation behavior. Training and evaluation remain simulation and
measurement infrastructure, not substitutes for unit tests.

| Command | Purpose |
| --- | --- |
| `stack build` | Compile the library and all four executables with configured warnings |
| `stack test` | Build and run the 59 Tasty/HUnit unit tests |
...
- Quantitative tables in `docs/experiments.md` were derived from local reports
  under ignored `outputs/`; the raw reports are not distributed by Git.
